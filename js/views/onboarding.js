...
export async function abrirOnboarding({ onDone, forzado = false } = {}) {
  ...
  try {
    ...
    console.warn('[olbo] no se pudo abrir la guía de inicio:', err);
    return;
  }
  ...
  raiz.setAttribute('aria-label', 'Guía de inicio de olbo');
  ...
  async function terminar() {
    try {
      ...
      console.warn('[olbo] no se pudo marcar la guía como completada:', err);
    }
    cerrar();
  ...
  const SLIDES = [
    { img: '10-piggy', title: 'Que no se te <b>escape</b> ni un peso.',
      text: 'olbo te da el control de tu plata en tiempo real. Cada gasto queda registrado antes de que se te olvide.' },
    { img: '15-coins', title: '<b>Captura</b> el gasto en 3 segundos.',
      text: 'Háblale o tómale una foto al recibo y olbo lo anota por ti. Ni una compra se te vuelve a escapar.' },
  ...
  function pasoNombre() {
    return {
      html: `
        ...
        <p class="ob__text">Para personalizar tu olbo. Solo tu nombre, nada más.</p>
        <label class="field">
  ...
  function pasoFijos() {
    ...
    const cats = CATEGORIAS.map((c) => `<option value="${esc(c.id)}"${c.id === 'vivienda' ? ' selected' : ''}>${esc(c.label)}</option>`).join('');
    ...
    return {
      html: `
        ...
        <p class="ob__text">Tu checklist de gastos fijos: arriendo, colegio, seguros, suscripciones… y también los que cambian de valor (luz, agua, gasolina, celular). olbo los tiene en cuenta al calcular lo que te queda.</p>
        <p class="ob__text ob__text--sm">Si el monto es siempre igual, ponlo. Si cambia cada mes, márcalo como <strong>valor variable</strong> y olbo te preguntará el valor real cada mes. Este paso es opcional, puedes saltarlo.</p>
        ${lista}
        ${form}`,
      foot: `
        <div class="ob__actions">
  ...
  function pasoListo() {
    ...
    return {
      ...
      foot: `
        <div class="ob__actions">
          <button type="button" class="btn btn--primary btn--block" data-act="terminar">Ir a olbo</button>
        </div>`,
      bind() {},
    };
  }

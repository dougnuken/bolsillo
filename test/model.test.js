...
test('crearIngreso negocio: nombre libre, montoEsperado opcional y crédito vinculado', () => {
  const neg = crearIngreso({ fuente: 'negocio', nombre: 'Ingreso Demo', diaDelMes: 15, montoEsperado: 1_500_000, creditoId: 'cred-1' });
  assert.equal(neg.fuente, 'negocio');
  assert.equal(neg.nombre, 'Ingreso Demo');
...
test('migrarIngresos: respeta un nombre ya puesto y migra negocio2 con su etiqueta', () => {
  const conNombre = { id: 'a', fuente: 'negocio1', monto: 800_000, diaDelMes: 5, nombre: 'Negocio Demo' };
  const sinNombre = { id: 'b', fuente: 'negocio2', monto: 400_000, diaDelMes: 10, nombre: '' };
  const [a, b] = migrarIngresos([conNombre, sinNombre]);
  assert.equal(a.nombre, 'Negocio Demo');
...
test('migrarIngresos: NO toca empleo ni las fuentes ya nuevas', () => {
  ...
  const negocioNuevo = crearIngreso({ fuente: 'negocio', nombre: 'Ingreso Demo', diaDelMes: 15 });
...
test('validarCredito: un crédito viejo SIN campo activo sigue siendo válido', () => {
  const viejo = {
    entidad: 'Banco Demo', producto: 'Libre inversión', tipo: 'Libre inversión',
...
const creditoMinimo = { entidad: 'Banco Demo', producto: 'Libre inversión', cuotaMensual: 850000 };
...
test('crearCredito: guarda con saldo, tasa y día VACÍOS (los completa la AI)', () => {
  ...
  assert.equal(c.entidad, 'Banco Demo');
...
test('crearCredito: exige producto (fail-fast, no guarda a medias)', () => {
  assert.throws(
    () => crearCredito({ entidad: 'Banco Demo', cuotaMensual: 850000 }),
...
test('crearCredito: varios productos de la MISMA entidad conviven', () => {
  const libre = crearCredito({ entidad: 'Banco Demo', producto: 'Libre inversión', cuotaMensual: 850000 });
  const visa = crearCredito({ entidad: 'Banco Demo', producto: 'Tarjeta Visa', cuotaMensual: 320000 });
  assert.notEqual(libre.id, visa.id);
  assert.equal(libre.entidad, visa.entidad);
  assert.notEqual(libre.producto, visa.producto);

  const delBanco = [libre, visa].filter((c) => c.entidad === 'Banco Demo');
...
/* ---------------- crédito: identidad de tarjeta (cartera) ---------------- */

test('crearCredito: guarda últimos 4, franquicia y skin de tarjeta', () => {
  const c = crearCredito({
    entidad: 'Banco Demo', producto: 'Tarjeta Negra', cuotaMensual: 500000,
    ultimosCuatro: '4111', franquicia: 'mastercard', skin: 'grafito',
  });
  assert.equal(c.ultimosCuatro, '4111');
  assert.equal(c.franquicia, 'mastercard');
  assert.equal(c.skin, 'grafito');
  assert.ok(FRANQUICIAS.includes(c.franquicia) && SKINS_TARJETA.includes(c.skin));
});

test('crearCredito: sin identidad de tarjeta → los campos quedan en null (crédito sin plástico)', () => {
  const c = crearCredito({ entidad: 'Banco Uno', producto: 'Crédito', cuotaMensual: 300000 });
  assert.equal(c.ultimosCuatro, null);
  assert.equal(c.franquicia, null);

...
function credito(over = {}) {
  return crearCredito({ entidad: 'Banco Demo', producto: 'Libre inversión', cuotaMensual: 850_000, ...over });
}
...
test('créditos sin ingresos de negocio: las cuotas pesan COMPLETAS en fijos', () => {
  // Arrange: sueldo 17M, dos créditos Banco Demo (850k + 320k), nada más.
  const creditos = [
...
test('resumenNegocios: cobertura, color y crédito vinculado por fuente', () => {
  ...
  const cTierra = credito({ entidad: 'Banco Demo', producto: 'Libre inversión', cuotaMensual: 850_000 });
  const cDC = credito({ entidad: 'Bancolombia', producto: 'Tarjeta', cuotaMensual: 320_000 });
  const fTierra = crearIngreso({ fuente: 'negocio', nombre: 'Ingreso Demo', diaDelMes: 15, creditoId: cTierra.id });
  const fDC = crearIngreso({ fuente: 'negocio', nombre: 'Negocio Demo', diaDelMes: 20, creditoId: cDC.id, montoEsperado: 400_000 });
  const movs = [
  ...
  // Assert
  const tierra = filas.find((f) => f.nombre === 'Ingreso Demo');
  ...
  assert.equal(tierra.creditoLabel, 'Banco Demo · Libre inversión');

  const dc = filas.find((f) => f.nombre === 'Negocio Demo');

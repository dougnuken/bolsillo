...
test('crearPrestamo: normaliza y congela; saldo = monto sin abonos', () => {
  const p = crearPrestamo({ persona: '  Bea ', concepto: ' Prepagada ', monto: 400000 }, { now: NOW });
  assert.equal(p.persona, 'Bea');
...
test('agregarAbono: baja el saldo y suma abonado', () => {
  let p = crearPrestamo({ persona: 'Bea', monto: 400000 }, { now: NOW });
...
test('totalPorCobrar y resumenPrestamos separan activos de saldados', () => {
  const a = crearPrestamo({ persona: 'Bea', monto: 400000 }, { now: NOW });
  ...
  // activos ordenados por mayor saldo: Bea (400k) antes que Leo (200k)
  assert.equal(r.activos[0].persona, 'Bea');
...
test('rehidratar desde datos guardados preserva id y abonos válidos', () => {
  const p = crearPrestamo({
    id: 'pre-fijo', persona: 'Bea', monto: 400000,
